  ...
  async function fetchProject() {
    try {
      const { data, error } = await (supabase
        ...
        .select(`
          *,
          profiles!user_id (id, full_name, university_id, username),
          cohorts (
            id,
            title,
            prize_pool,
      ...
      if (data && data.profiles?.username && data.project_slug) {
        router.replace(projectPath(data.profiles.username, data.project_slug));
        return;
      }
  ...
  async function handleVote() {
    ...
    try {
      if (isVoted) {
        ...
        // insert vote (upvote)
        await upvoteProject(id, user.id);
        setIsVoted(true);
        setVoteCount((c) => c + 1);
      }
    } catch (err) {
    ...
    <>
      ...
      <main style={{ minHeight: "100svh", paddingTop: "100px", paddingBottom: "96px", backgroundColor: "#0b0c0f" }}>
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ fontSize: "0.8125rem", color: "#666666", margin: "12px 0 0 0" }}>
                  Shipped by <span style={{ color: "#888888", fontWeight: 500 }}>{project.profiles?.full_name || "Anonymous"}</span>
                  {project.cohorts?.universities?.name && ` · ${project.cohorts.universities.name}`}
                </p>
              </div>

              {/* Voting & Action Side Column */}
              <div
        ...
        <div style={containerStyle}>
          ...
          >
            {/* Desktop columns override logic (standard responsive styles applied using CSS style sheet overrides on width tags) */}
            <div
              style={{
                display: "flex",
                flexDirection: "row",
